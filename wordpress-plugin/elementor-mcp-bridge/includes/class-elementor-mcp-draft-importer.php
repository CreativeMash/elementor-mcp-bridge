...
final class Elementor_MCP_Draft_Importer {
	...
	private static function convert_node( array $node ): ?array {
		...
		return array( 'id' => self::id( $node['id'] ?? wp_generate_uuid4() ), 'elType' => 'container', 'isInner' => true, 'settings' => array_merge( self::visual( $node ), array( 'content_width' => 'full', 'flex_direction' => 'HORIZONTAL' === ( $node['layoutMode'] ?? '' ) ? 'row' : 'column', 'flex_justify_content' => $justify[ $node['primaryAxisAlignItems'] ?? '' ] ?? 'flex-start', 'flex_align_items' => $align[ $node['counterAxisAlignItems'] ?? '' ] ?? 'stretch', 'flex_gap' => isset( $node['itemSpacing'] ) ? array( 'column' => (string) $node['itemSpacing'], 'row' => (string) $node['itemSpacing'], 'isLinked' => true, 'unit' => 'px', 'size' => (float) $node['itemSpacing'] ) : null, 'padding' => self::box( $node['paddingTop'] ?? 0, $node['paddingRight'] ?? 0, $node['paddingBottom'] ?? 0, $node['paddingLeft'] ?? 0 ), 'width' => self::size( $node['absoluteBoundingBox']['width'] ?? null ), 'min_height' => 'FIXED' === ( $node['layoutSizingVertical'] ?? '' ) ? self::size( $node['absoluteBoundingBox']['height'] ?? null ) : null ) ), 'elements' => $children );
	}
